    ...
    try:
        ...
        if not parent_dir.exists():
            try:
                ...
                raise GitAIError(f"Permission denied: Cannot create directory {parent_dir}")
            except OSError as e:
...
def suggest_similar_templates(template_name: str, available_templates: List[str]) -> List[str]:
    ...
    if not suggestions:
        for template in available_templates:
            ...
            if (template_lower.startswith(template_lower_full[:3]) or 
                template_lower.endswith(template_lower_full[-3:]) or
                template_lower_full.startswith(template_lower[:3]) or
                template_lower_full.endswith(template_lower[-3:])):
                suggestions.append(template)
